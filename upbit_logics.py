...
def calc_all_tickers_indicators():
    ...
    for i, key in enumerate(BOOK):
        ...
        ticker, unit = key.split(',')
        tu = f"[ {ticker}, {unit} ]"
        print(f"Calculate Indicators \t  -\t{i+1: 3d} / {len(BOOK)} working on : {tu:23s}")
...
def evaluate_all_tickers():
    ...
    for i, key in enumerate(BOOK.keys()):
        ...
        ticker, unit = key.split(',')
        tu = f"[ {ticker}, {unit} ]"
        print(f"Evaluate status \t  -\t{i+1: 3d} / {len(BOOK)} working on : {tu:23s}")
...
def read_evals_only():
    ...
    for key in BOOK:
        ...
        if RSI_status.startswith("과매도"):
            RSI_status = txt_color.RED + RSI_status + txt_color.ENDC
        elif RSI_status.startswith("과매수"):
            RSI_status = txt_color.BLUE + RSI_status + txt_color.ENDC

        # MACD
        MACD_status = BOOK[key]["eval"]["MACD"]
        if MACD_status.startswith("골든크로스"):
            MACD_status = txt_color.RED + MACD_status + txt_color.ENDC
        elif MACD_status.startswith("데드크로스"):
            MACD_status = txt_color.BLUE + MACD_status + txt_color.ENDC

        # Williams %R
        WilliamsR_status = BOOK[key]["eval"]["Williams%R"]
        if WilliamsR_status.startswith("과매도"):
            WilliamsR_status = txt_color.RED + WilliamsR_status + txt_color.ENDC
        elif WilliamsR_status.startswith("과매수"):
            WilliamsR_status = txt_color.BLUE + WilliamsR_status + txt_color.ENDC

        line = ""
        line += f"> {key:20s}\t"
        line += f"RSI: {RSI_status:15s}\t"
        line += f"MACD: {MACD_status:15s}\t"
        line += f"Williams %R: {WilliamsR_status:15s}"
